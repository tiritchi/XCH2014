<?php
	...
	function get_user_info($bdd,$user_id){
		...
		$donnees = $req->fetch();
		$tab = array(substr($donnees['pseudo'],1,(strlen($donnees['pseudo'])-2)),substr($donnees['fname'],1,(strlen($donnees['fname'])-2)),substr($donnees['lname'],1,(strlen($donnees['lname'])-2)),substr($donnees['school'],1,(strlen($donnees['school'])-2)),substr($donnees['mail'],1,(strlen($donnees['mail'])-2)),substr($donnees['sexe'],1,(strlen($donnees['sexe'])-2)),substr($donnees['date_naissance'],1,(strlen($donnees['date_naissance'])-10)),"0".$donnees['phone'],substr($donnees['adresse'],1,(strlen($donnees['adresse'])-2)));
		return $tab;
	}
